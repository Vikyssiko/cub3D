...
t_dist	find_h_dist(t_game **game, t_ray *ray, double angle, t_dist dist, int door)
{
	double	distance;
	char	**map;
...
//			break ;
//		}
		if (door && map[(int)(ray->y) >> BITS][(int)(ray->x) >> BITS] == 'D')
		{
...
//		else if (door && map[(int)(ray->y) >> BITS][(int)(ray->x) >> BITS] == 'D')
//		{
//			distance = cos(angle) * (ray->x - (*game)->player_x + ray->x_diff / 2)
//					   - sin(angle) * (ray->y - (*game)->player_y + ray->y_diff / 2);
//			dist.dist = find_min(dist.dist, distance);
...
//		ray.y = (*game)->player_y;
//	}
	return (find_h_dist(game, &ray, angle, dist, door));
}
